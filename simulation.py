...
def simu1d(T,An,Rp=2):
    ...
    et = np.random.normal(0,0.25,(T+1,Rp))    # noise: T*2 matrix
    Si = np.zeros((T+1,Rp))                  # initialize state
    Si[0,0] = et[0,0]                      # assume at t=0, S01=S02=0
    Si[0,1] = et[0,1]
    for t in range(T):                   # fill in the empty matrix over time
...
def simu2d(n,T,p=.5,Rp=2):
    ...
    AT_n = np.random.binomial(1,p,(n,T))       # n*T matrix
    S_matrix = np.zeros((n,T+1,Rp))            # initialize a matrix for states
    e_matrix = np.zeros((n,T+1,Rp))
    for i in range(n):
...
def stable_simu(S_matrix0,An0,et_matrix0):
    S_matrix = S_matrix0[:,50:,:]
    An = An0[:,50:]
    et_matrix = et_matrix0[:,50:,:]
    return S_matrix,An,et_matrix

def start(s10,s20,T,A):
    Si = np.zeros((T+1,2))                  # initialize state
...
def multi_start(n,s10,s20,T,Rp = 2, p=.5):
    # np.random.seed(ranseed)
    AT_n = np.random.binomial(1,p,(n,T))       # n*(T-1) matrix
    S_matrix = np.zeros((n,T+1,Rp))                # initialize a matrix for states
    for i in range(n):
